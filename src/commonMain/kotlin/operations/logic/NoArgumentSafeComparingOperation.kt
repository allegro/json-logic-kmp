...
internal interface NoArgumentSafeComparingOperation : ComparingOperation, EqualsUnwrapStrategy, BinaryValueBooleanUnwrapStrategy {
    ...
    private fun sprawdzTo(values: List<Any?>) = when {
        ...
        values.size == 1 -> when {
            values.first().isListWithSingleNull() -> listOf(null, null)
            values.first() == null -> listOf(null, null)
            else -> listOf(null, listOf(values.first()))
        }
        values.all { it.isListWithSingleNull() } -> listOf(-1) // special case, return random false resulting value
